...
colorama.init()
################################################################################
banner=Fore.RED+'''(っ◔◡◔)っ MADE BY 999                           
'''+Fore.RESET
credit=(Fore.RED+
'''Please Go Support My Youtube
   YOUTUBE = 999RAIDS                                            
'''+Fore.RESET)
################################################################################
#platform info
...
def update():
	...
	if version>de_version:
		...
		time.sleep(3)
		webbrowser.open('https://youtube.com/channel/UCZLubapQSYe5RRnETTncppA')
		exit()
	else:
		pass
try:
	print(Fore.CYAN+" Starting It Up! "+Fore.RESET)
	time.sleep(2)
	chech_con()
	update()
	os.system(cmd)
...
try:
	while True:
		...
		opt=str(input(Fore.RED+"\nEnter Your choice: "+Fore.RESET))
		if opt=='1':
			domain=str(input(Fore.BLUE+"Enter The Website (eg:google.com):"+Fore.RESET))
			ip=socket.gethostbyname(domain)
			break
		elif opt=='2':
			ip = input(Fore.GREEN+"IP Address  : "+Fore.RESET)
			break
		elif opt=='3':
			time.sleep(1)
			print(Fore.RED+"bye bye o/"+Fore.RESET)
			exit()
		else:
			print(Fore.RED+'Invaild Choice!'+Fore.RESET)
			time.sleep(2)
			os.system(cmd)
	port =int(input(Fore.BLUE+"Port : "+Fore.RESET))
	os.system(cmd)
